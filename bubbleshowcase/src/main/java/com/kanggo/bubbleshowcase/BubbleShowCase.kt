...
class BubbleShowCase(builder: BubbleShowCaseBuilder){
        ...
        if (mTargetView != null && mArrowPositionList.size <= 1) {
            ...
            handler.postDelayed({
                ...
                if(mArrowPositionList.isEmpty()){
                    if(ScreenUtils.isViewLocatedAtHalfTopOfTheScreen(mActivity, target)) mArrowPositionList.add(ArrowPosition.TOP) else mArrowPositionList.add(ArrowPosition.BOTTOM)
                    bubbleMessageViewBuilder = getBubbleMessageViewBuilder()
                }
    ...
    private fun getBackgroundDimLayout(): RelativeLayout {
        ...
        backgroundLayout.setBackgroundColor(ContextCompat.getColor(mActivity, R.color.transparent_grey))
    ...
    private fun addTargetViewAtBackgroundDimLayout(targetView: View?, backgroundDimLayout: RelativeLayout?) {
        ...
        targetViewParams.setMargins(getXposition(targetView), getYposition(targetView), getScreenWidth(mActivity) - (getXposition(targetView) + targetView.width), 0)
            ...
            ArrowPosition.LEFT -> {
                ...
                if(ScreenUtils.isViewLocatedAtHalfTopOfTheScreen(mActivity, targetView)){
                    showCaseParams.setMargins(
                            ...
                            if(isTablet()) getScreenWidth(mActivity) - (getXposition(targetView) + targetView.width) - getMessageViewWidthOnTablet(getScreenWidth(mActivity) - (getXposition(targetView) + targetView.width)) else 0,
                            0)
            ...
            ArrowPosition.TOP -> {
                ...
                if(ScreenUtils.isViewLocatedAtHalfLeftOfTheScreen(mActivity, targetView)){
                    showCaseParams.setMargins(
                            ...
                            if (isTablet()) getScreenWidth(mActivity) - getXposition(targetView) - getMessageViewWidthOnTablet(getScreenWidth(mActivity) - getXposition(targetView)) else 0,
                            0)
                } else{
                    showCaseParams.setMargins(
    ...
    private fun addBubbleMessageViewOnScreenCenter(bubbleMessageViewBuilder: BubbleMessageView.Builder, backgroundDimLayout: RelativeLayout?) {
        ...
        if(isTablet()) showCaseParams.setMargins(
                if (isTablet()) getScreenWidth(mActivity)/2 - ScreenUtils.dpToPx(MAX_WIDTH_MESSAGE_VIEW_TABLET)/2 else 0,
                0,
                if (isTablet()) getScreenWidth(mActivity)/2 - ScreenUtils.dpToPx(MAX_WIDTH_MESSAGE_VIEW_TABLET)/2 else 0,
                0)
        val animation = AnimationUtils.getScaleAnimation(0, DURATION_SHOW_CASE_ANIMATION)
